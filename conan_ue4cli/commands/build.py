...
def build(manager, argv):
	...
	parser.add_argument('--export-only', action='store_true', help='Export package recipes to the local Conan cache but do not build binaries')
	...
	# If the recipe cache directory does not exist (usually because this is our first build), then populate it
	cacheDir = RecipeCache.getCacheDirectory()
	if exists(cacheDir) == False:
		update(manager, argv)
	...
	# (Keep track of the list of exported package names in case the user asked us to build all of them)
	exported = []
	sources = [cacheDir, os.getcwd()] + (args.sources if args.sources is not None else [])
	...
	# If we are only exporting without building binaries then stop here
	if args.export_only == True:
		print('Exports completed, skipping build phase.', flush=True)
		return
